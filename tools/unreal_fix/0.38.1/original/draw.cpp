...
//   temp.scx = 384, temp.scy = 300;
   ...
   unsigned width = temp.scx/4; // Ширина экрана в знакоместах * 2 (т.к. используются пиксели и атрибуты на 1 знакоместо)
   //temp.vidbufsize = temp.scx*temp.scy/4;

   // make video table
   unsigned mid_lines = 192; // Число строк в центральной части экрана (без бордюра)
   unsigned buf_mid = 256; // Число пикселей в центральной части экрана (без бордюра)

   // Расчет размер бордюра в пикселях (1 пиксель = 2 такта)
   temp.b_bottom = temp.b_top = 24, temp.b_left = 32; // border small
   if (conf.bordersize==0) temp.b_top = temp.b_left = 0; // border none
   if (conf.bordersize==2) temp.b_top = temp.b_left = 64; // border full
   ...
   // 384x300 - border full
   temp.b_right = temp.scx - buf_mid - temp.b_left; // Расчет размера правой части бордюра в пикселях (1 пиксель = 2 такта)
   temp.b_bottom = temp.scy - mid_lines - temp.b_top; // Расчет размера нижней части бордюра в пикселях (1 пиксель = 2 такта)
   ...
   // (включает невидимую часть + полностью верхний бордюр + начало строки шириной в левый бордюр)
   unsigned t = conf.paper - temp.b_top*conf.t_line; // Число тактов до начала верхнего бордюра (включая левый бордюр в первой строке paper)
   video[inx++].next_t = ts(t - temp.b_left/2); // Исключение левого бордюра (next_t - такт начала первой строки верхнего бордюра)
   for (i = 0; i < temp.b_top; i++)
   { // верхний бордюр
      video[inx].next_t = ts(t + (buf_mid+temp.b_right)/2); // Конец правого бордюра
      video[inx].screen_ptr = rbuf+width*i;
      video[inx].nextvmode = 0; // Следующая точка - начало следующей строки

       // Переход к следующей строке (t - такт следующей строки + левый бордюр)
      inx++; t += conf.t_line;
      video[inx++].next_t = ts(t - temp.b_left/2); // Исключение левого бордюра (такт начала следующей строки)
   }
   for (i = 0; i < mid_lines; i++)
   { // левый бордюр + экран + правый бордюр
      video[inx].next_t = ts(t); // Конец левого бордюра
      video[inx].screen_ptr = rbuf+width*(i+temp.b_top);
      video[inx].nextvmode = 2; // Далее прорисовывается screen

      inx++;
      video[inx].next_t = ts(t + buf_mid/2); // Конец screen
