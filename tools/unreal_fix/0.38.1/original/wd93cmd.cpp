   ...
   for (;;)
   {
      ...
      switch (state)
      {
         ...
         case S_DELAY_BEFORE_CMD:
            if (!conf.wd93_nodelay && (cmd & CMD_DELAY))
        ...
        nextmk:
         ...
         case S_TYPE1_CMD: // Подана команда типа 1 (restore/seek/step)
            status &= ~(WDS_DRQ | WDS_CRCERR | WDS_SEEKERR | WDS_WRITEP);
            rqs = 0;
         ...
         case S_STEP:
         {
            ...
            if (!(cmd & 0xF0) && (seldrive->track == 0)) // Команда restore, проверка TRK00
            {
                track = 0;
            ...
            if (!(cmd & 0xE0) || (cmd & CMD_SEEK_TRKUPD)) // Команда seek
                track += stepdirection;
            ...
            state2 = (cmd & 0xE0) ? S_VERIFY /*Команда step*/: S_SEEK /*Команда seek*/;
            state = S_WAIT;
            break;
         }
