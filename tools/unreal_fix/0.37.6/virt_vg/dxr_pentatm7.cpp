...
// render ZXEVO textmode #7


/* was for atm #6 textmode
const int text0_ofs = 0x0;                  // lsym
const int text1_ofs = 0x2000;               // rsym
const int text2_ofs = -4*int(PAGE)+1;       // rattr
const int text3_ofs = -4*int(PAGE)+0x2000;  // lattr
*/

// for ZXEVO #7 textmode (pages 8 or 10)
const int text0_ofs = 3*int(PAGE) + 0x0000; // lsym
const int text1_ofs = 3*int(PAGE) + 0x1000; // rsym
const int text2_ofs = 3*int(PAGE) + 0x2001; // rattr
const int text3_ofs = 3*int(PAGE) + 0x3000; // lattr


// all code below copypasted from dxr_atm6.cpp

void line_pentatm7_8(unsigned char *dst, unsigned char *src, unsigned *tab0, unsigned char *font, int src_offset)
{
...
void line_pentatm7_16(unsigned char *dst, unsigned char *src, unsigned *tab0, unsigned char *font, int src_offset)
{
...
void line_pentatm7_32(unsigned char *dst, unsigned char *src, unsigned *tab0, unsigned char *font, int src_offset)
{
...
// Textmode
void rend_pentatm7(unsigned char *dst, unsigned pitch, int y, int Offset)
{
   ...
   if (temp.scy > 200) 
    ...
    if (conf.fast_sl) {
        ...
        {
        case 8:
            line_pentatm7_8 (dst2, temp.base, t.zctab8 [0], fontatm2 + v*0x100, Offset); 
            break;
        case 16:
            line_pentatm7_16(dst2, temp.base, t.zctab16[0], fontatm2 + v*0x100, Offset); 
            break;
        case 32:
            line_pentatm7_32(dst2, temp.base, t.zctab32[0], fontatm2 + v*0x100, Offset); 
            break;
        }
    } else {
        dst2 += 2*y*pitch;
        ...
        case 8:
            line_pentatm7_8 (dst2, temp.base, t.zctab8 [0], fontatm2 + v*0x100, Offset); 
            dst2 += pitch;
            line_pentatm7_8 (dst2, temp.base, t.zctab8 [1], fontatm2 + v*0x100, Offset);
            break;
        case 16:
            line_pentatm7_16(dst2, temp.base, t.zctab16[0], fontatm2 + v*0x100, Offset); 
            dst2 += pitch;
            line_pentatm7_16(dst2, temp.base, t.zctab16[1], fontatm2 + v*0x100, Offset);
            break;
        case 32:
            line_pentatm7_32(dst2, temp.base, t.zctab32[0], fontatm2 + v*0x100, Offset); 
            dst2 += pitch;
            line_pentatm7_32(dst2, temp.base, t.zctab32[1], fontatm2 + v*0x100, Offset);
            break;
        }
    }
}
